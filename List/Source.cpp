...
expression::expression(char* input) {
	int pos = 0, len_input = strlen(input);
	char *symbol, *buffer;
	stack OperatorStk;
	bool abs_flag = false;
	var = NULL;
	do {
		symbol = input + pos++;
		if (test_on_symbol(symbol)
			|| test_on_unar_minus(input, pos, abs_flag)
			&& (*symbol) != '\0') {
			int len = 1;
			bool flag_for_point = false;

			if (test_on_figure(symbol)
				|| ((*symbol) == '-')) {
...
double expression::counting() {
	...
	while (ExpStack.see())
	{
		char* tmp = new char[2];
		tmp[1] = '\0';
		tmp[0] = *ExpStack.look_down();
		int point = 0;
		int sign = 1;
		if (tmp[0] == '-') {
			if (test_on_symbol(ExpStack.look_down() + 1)) {
				tmp[0] = ' ';
				sign = -1;
				point = 1;
				tmp[0] = *(ExpStack.look_down() + 1);
			}
		}
		if (test_on_symbol(tmp)) {
			if (test_on_figure(tmp)) {
				st.push(atof(ExpStack.look_down() + point)*sign);
			}
			else {
				st.push(find_el_var(ExpStack.look_down() + point)*sign);
			}
		}

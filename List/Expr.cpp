...
expression::expression(char* input) {
	...
	do {
		...
		if (test_on_symbol(symbol) 
			...
			if (test_on_figure(symbol) 
				...
				|| ((*symbol) == '-' && test_on_letter(symbol + 1)) ) {
	...
	while (*(input + pos - 1) == ' ') {	// wrote complex figure
		...
		len = 1;
		bool flag_for_point = false;
		while (test_on_figure(input + pos + len) 
			|| input[pos + len] == '.') {
			if (input[pos + len] == '.') {
				if (!flag_for_point) {
					flag_for_point = true;
